...
const AiSummarizeBox = () => {
  ...
  const handleApiCall = async () => {
    ...
    try {
      ...
      // 현재 작업 정보 업데이트
      console.log("🔄 updateSummarizeWork 호출 전:", { historyId, sequenceNumber });
      ...
      <div className={clsx("flex flex-col md:flex-row", "flex-1 rounded-lg shadow-lg overflow-hidden border bg-white")}>
        ...
        <div className="w-full h-1/2 md:h-full md:w-1/2 p-2 md:p-4 relative bg-gray-50">
          ...
          {(selectedHistory?.summarizedText || outputText) && (
            <button onClick={() => navigator.clipboard.writeText(selectedHistory?.summarizedText || outputText)} className="absolute top-3 right-3 p-2 text-gray-500 hover:bg-gray-200 rounded-full">
